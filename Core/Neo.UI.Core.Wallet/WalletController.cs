...
namespace Neo.UI.Core.Wallet
{
    internal class WalletController : IWalletController
    {
        ...
        public void Relay(Transaction transaction, bool saveTransaction = true)
        {
            ...
            this.blockchainService.Relay(transaction);
        ...
        public void Relay(IInventory inventory)
        {
            ...
            this.blockchainService.Relay(inventory);
        }
        ...
        public IEnumerable<Coin> FindUnspentCoins()
        {
            ...
            var coins = this.currentWallet.FindUnspentCoins();
        ...
        public bool CanViewCertificate(string assetIdStr)
        {
            ...
            var assetSummary = this.currentWalletInfo.GetFirstClassAsset(assetId);

            if (assetSummary == null || assetSummary.IsSystemAsset || assetSummary.AssetOwner == null) return false;

            var queryResult = this.GetCertificateQueryResult(assetSummary.AssetOwner);
        ...
        public string ViewCertificate(string assetIdStr)
        {
            if (!UInt256.TryParse(assetIdStr, out var assetId)) return null;

            var asset = this.currentWalletInfo.GetFirstClassAsset(assetId);
        ...
        public Fixed8 CalculateUnavailableBonusGas(uint height)
        {
            ...
            var unspent = this.FindUnspentCoins().Where(p =>p.Output.AssetId.Equals(this.blockchainService.GoverningTokenHash)).Select(p => p.Reference);
        ...
        public string GetNEP5TokenAvailability(string scriptHashStr)
        {
            ...
            var asset = this.currentWalletInfo.GetNEP5Asset(scriptHash);
        ...
        public void DeleteFirstClassAsset(string assetIdStr)
        {
            this.ThrowIfWalletIsNotOpen();
            ...
            this.BuildSignAndRelayTransaction(transactionParameters);
        }
        ...
        public void ClaimUtilityTokenAsset()
        {
            ...
            var claimingAssetId = this.blockchainService.UtilityTokenHash;
        ...
        public bool DeleteAccount(string accountScriptHash)
        {
            this.ThrowIfWalletIsNotOpen();
        ...
        private void OnBlockAdded(object sender, EventArgs e)
        {
            ...
            if (coins.Any(coin => !coin.State.HasFlag(CoinState.Spent) &&
                                  coin.Output.AssetId.Equals(this.blockchainService.GoverningTokenHash)))
            {
                this.SetWalletBalanceChangedFlag();
            }
        ...
        private Fixed8 CalculateBonus(IEnumerable<CoinReference> inputs, uint heightEnd)
        {
        ...
        private void AddAccountItem(WalletAccount account)
        {
            ...
            var newAccountItem = new AccountSummary(account.Label, account.ScriptHash, accountType);
        ...
        private void UpdateAccountBalances()
        {
            ...
            var balanceNeo = coins.Where(p => p.Output.AssetId.Equals(this.blockchainService.GoverningTokenHash)).GroupBy(p => p.Output.ScriptHash).ToDictionary(p => p.Key, p => p.Sum(i => i.Output.Value));
            var balanceGas = coins.Where(p => p.Output.AssetId.Equals(this.blockchainService.UtilityTokenHash)).GroupBy(p => p.Output.ScriptHash).ToDictionary(p => p.Key, p => p.Sum(i => i.Output.Value));
            ...
            foreach (var account in accountsList)
            {
                ...
                account.BalanceSummary.Neo = neo;
                account.BalanceSummary.Gas = gas;
            }
            ...
            if (this.GetWalletBalanceChangedFlag())
            {
                ...
                var confirmedGoverningTokens = coins
                    .Where(p => 
                        ...
                        p.Output.AssetId.Equals(this.blockchainService.GoverningTokenHash))
                    .Select(p => p.Reference);

                // TODO Issue #158 [AboimPinto]: Calculation of the Bonus is returning an Error
                //var walletIsSynchronized = this.WalletIsOpen && this.currentWallet.WalletHeight > this.blockchainService.BlockHeight + 1;
                //
                //if (!walletIsSynchronized)
                //{
                //    var lastBlockHeight = this.blockchainService.BlockHeight + 1;
                //
                //    var bonusUnavailable = this.blockchainService.CalculateBonus(confirmedGoverningTokens, lastBlockHeight);
                ...
                var assetDictionary = coins.GroupBy(p => p.Output.AssetId, (k, g) => new
                {
                    ...
                    Claim = k.Equals(this.blockchainService.UtilityTokenHash) ? bonus : Fixed8.Zero
                }).ToDictionary(p => p.Asset.AssetId);

                if (bonus != Fixed8.Zero && !assetDictionary.ContainsKey(this.blockchainService.UtilityTokenHash))
                {
                    assetDictionary[this.blockchainService.UtilityTokenHash] = new
                    {
                        Asset = this.blockchainService.GetAssetState(this.blockchainService.UtilityTokenHash),
                        Value = Fixed8.Zero,
                        Claim = bonus
                    };
                }
                ...
                foreach (var asset in this.currentWalletInfo.GetFirstClassAssets())
                {
                    ...
                    this.currentWalletInfo.RemoveFirstClassAsset(asset.AssetId);
                }
                ...
                foreach (var asset in assetDictionary.Values)
                {
                    ...
                    if (asset.Asset.AssetId.Equals(this.blockchainService.UtilityTokenHash))
                    {
                    ...
                    else
                    {
                        ...
                        var assetItem = new FirstClassAssetSummary(
                            asset.Asset.AssetId,
                            asset.Asset.Owner,
                        ...
                        var isSystemAsset = asset.Asset.AssetType == AssetType.GoverningToken ||
                                            asset.Asset.AssetType == AssetType.UtilityToken;

                        this.currentWalletInfo.AddFirstClassAsset(assetItem);
                        this.messagePublisher.Publish(new AssetAddedMessage(asset.Asset.AssetId.ToString(), assetName, $"{Strings.UnknownIssuer}[{asset.Asset.Owner}]", asset.Asset.AssetType.ToString(), TokenType.FirstClassToken, isSystemAsset, valueText));
                    }
                }
            ...
            foreach (var nep5ScriptHash in this.nep5WatchScriptHashes)
            {
                ...
                if (item != null)
                {
                    if (totalBalanceBigInt == BigInteger.Zero)
                    {
                        ...
                        this.currentWalletInfo.RemoveNEP5Asset(nep5ScriptHash);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if (totalBalanceBigInt != BigInteger.Zero)
                    {
                        var assetItem = new NEP5AssetSummary(nep5ScriptHash)
                        {
                            TotalBalance = totalBalance.ToString()
                        };

                        this.currentWalletInfo.AddNEP5Asset(assetItem);

                        // TODO Get NEP-5 token name
                        this.messagePublisher.Publish(new AssetAddedMessage(nep5ScriptHash.ToString(), "NEP-5 Token", $"ScriptHash:{nep5ScriptHash}", "NEP-5", TokenType.NEP5Token, false, totalBalance.ToString()));
                    }
                }
            }
        ...
        {
            var existingTransaction = this.currentWalletInfo.GetTransaction(transaction.Hash);
            
            if (existingTransaction != null) return;

            this.currentWalletInfo.AddTransaction(transaction);
            this.messagePublisher.Publish(new TransactionAddedMessage(transaction.Hash.ToString(), transaction.Time, transaction.Height, transaction.Type.ToString()));
        }
